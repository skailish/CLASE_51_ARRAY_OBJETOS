...
const empleadesConMasLenguajes = (cantidadLenguajes, empleades) => {
    const tieneMasLenguajesQue = empleade => empleade.lenguajes.length > cantidadLenguajes;
...
const sueldoPromedioEmpleades = (empleades) => {
    const sumaTotalSueldos = (sumaParcial, empleade) => {
        sumaParcial += empleade.sueldo;

        return sumaParcial
    }
    const totalSueldos = empleades.reduce(sumaTotalSueldos, 0);
    return Math.round((totalSueldos / empleades.length));
}
...
const sueldoPromedioPorSeniority = (seniorityFiltro, empleades) => {
    ...
    const empleadesConSeniority = empleade => empleade.seniority === seniorityFiltro;
...
const buscarEmpleades = (area, puesto, seniority, empleades) => {
    ...
    const filtrarEmpleades = empleade => empleade.area === area && empleade.puesto === puesto && empleade.seniority === seniority
...
const errorEnProduccion = empleades => {
    ...
    const cambiarATrue = empleade => {
        const cambioGuardia = { ...empleade } //// SI LO APLICO DIRECTO A EMPLEADE, CAMBIA EL ARRAY ORIGINAL

        cambioGuardia.haceGuardia = true

        return cambioGuardia
    };
...
const agregarTecnologias = empleades => {
    const copiaEmpleades = [...empleades];
    ...
    return copiaEmpleades.map(agregoTecnologias)
...
const empleadesQueSabenLenguajes = (lenguajes, empleades) => {
    ...
    const empleadeSabeLenguajes = (empleade, lenguajes) => {
        const lenguajesEmpleade = empleade.lenguajes;
        let tieneLenguajes = true;
        lenguajes.forEach(lenguaje => {
            tieneLenguajes = lenguajesEmpleade.includes(lenguaje) ? tieneLenguajes : false;
        });

        return tieneLenguajes
    }
...
const empleadesQueSabenAlgunosLenguajes = (lenguajes, empleades) => {
    ...
    const empleadeSabeAlgunLenguaje = (empleade) => {
        const lenguajesEmpleade = empleade.lenguajes;
        let tieneAlgunLenguaje = false;
        lenguajes.forEach(lenguaje => {
            tieneAlgunLenguaje = lenguajesEmpleade.includes(lenguaje) ? true : tieneAlgunLenguaje;
        });

        return tieneAlgunLenguaje
    }
...
const empleadesConMejorSueldo = empleades => {
    ...
    let i = 0;
    const top10sueldo = []
    while (top10sueldo.length <= 9) {
        top10sueldo.push(empleadesOrdenados[i])
        i++
    }
...
const obtenerTitulosCompletos = empleades => {
    ...
    const tituloCompleto = empleade => `${empleade.nombre}, ${empleade.seniority}, ${empleade.puesto}, ${empleade.area}`
...
const obtenerInfoPersonal = empleades => {
    ...
    const infoPersonal = empleade => ({ nombre: empleade.nombre, pais: empleade.pais, edad: empleade.edad })
...
const obtenerInfoPuestos = empleades => {
    ...
    const infoPuestos = empleade => ({ nombre: empleade.nombre, area: empleade.area, puesto: empleade.puesto, seniority: empleade.seniority })
...
const obtenerInfoSeniority = empleades => {
    ...
    const infoSeniority = empleade => ({ nombre: empleade.nombre, seniority: empleade.seniority, sueldo: empleade.sueldo, cantidadLenguajes: empleade.lenguajes.length })
...
const obtenerInfoPagos = empleades => {
    ...
    const infoPagos = empleade => ({ nombre: empleade.nombre, sueldoBruto: empleade.sueldo, obraSocial: Math.round((empleade.sueldo * 0.03)), jubilacion: Math.round((empleade.sueldo * 0.11)), sueldoNeto: Math.round((empleade.sueldo - (empleade.sueldo * 0.03) - (empleade.sueldo * 0.11))) })
...
const obtenerEstadisticasLenguajes = empleades => {
    const sumaLenguajes = (sumaParcial, empleade) => {
        const lenguajes = empleade.lenguajes;
        for (let i = 0; i < lenguajes.length; i++) {
            sumaParcial[lenguajes[i]] = sumaParcial[lenguajes[i]] + 1 || 1
        }
        return sumaParcial
    }
   return empleades.reduce(sumaLenguajes, {});
}
